...
//   };
//   const cls = map[status?.toLowerCase()] || "bg-amber-50 text-amber-700 border-amber-100";
//   return (
...
//             </span>
//             <span className="bg-white/15 text-white text-xs px-3 py-1 rounded-full font-medium border border-white/20">
//               Roll: {student.rollNo || student._id?.slice(-6)?.toUpperCase() || "N/A"}
//             </span>
//           </div>
//         </div>
//       </div>
...
//                 <InfoRow label="Roll No" value={student.rollNo || student._id?.slice(-6)?.toUpperCase()} />
//                 <InfoRow
//                   label="Joined"
//                   value={
//                     student.createdAt
...
//             <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
//               <div className="px-5 py-3.5 border-b border-slate-100 bg-slate-50">
//                 <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Project Info</h3>
//               </div>
//               {student.projectTitle ? (
//                 <div className="px-5">
//                   <InfoRow label="Project Title" value={student.projectTitle} />
//                   <InfoRow label="Project Status" value={student.project?.status || student.status} />
//                 </div>
//               ) : (
//                 <p className="text-sm text-slate-400 text-center py-8">No project assigned</p>
//               )}
//             </div>
//           </div>
//         )}

//         {activeTab === "Assignments" && <AssignmentsTeacherTab student={student} />}
...
//   const { assignedStudents, selectedStudent, loading } = useSelector((state) => state.teacher);
...
//       {/* Page Header */}
//       <div className="bg-white rounded-xl shadow-md p-6 flex flex-col md:flex-row items-center
...
//             <StudentCard key={student._id} student={student} onView={handleView} />
//           ))}
//         </div>
//       )}
//     </div>
...
const StudentsOverview = () => {
  ...
  useEffect(() => {
    dispatch(getAssignedStudents());
    // Clear any selected student on mount to prevent auto-opening
    dispatch(setSelectedStudent(null));
  }, [dispatch]);

  const handleView = (student) => {
    setActiveTab("Overview"); // Reset tab when opening student
      ...
      {selectedStudent ? (
        ...
        <>
          ...
          <div className="bg-white border-b border-slate-200 px-6 py-3.5 flex items-center gap-3 shadow-sm sticky top-0 z-10">
            ...
            <span className="text-slate-300">|</span>
            <span className="text-sm text-slate-400 truncate">{selectedStudent.name}</span>
          </div>
          ...
          <div className="p-6 bg-slate-50 min-h-[400px]">
            ...
            {activeTab === "Chat" && (
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-8 text-center">
                <p className="text-slate-400">Chat feature coming soon</p>
              </div>
            )}
          </div>
        </>
