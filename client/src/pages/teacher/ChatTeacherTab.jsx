...
const ChatTeacherTab = ({ student }) => {
  ...
  useEffect(() => {
    if (student?._id) {
      // Backend se purane messages fetch karein
      dispatch(getMessages(student._id));

      // Socket connection aur room join logic
      socket.connect();
      socket.emit("join", { userId: authUser._id });

      // Real-time message listener
      socket.on("newMessage", msg => {
        // Sirf wahi messages dikhao jo is current student ke hain
        if (
  ...
  const handleSend = async e => {
    ...
    // Redux action dispatch karein (Ye backend API ko hit karega)
    dispatch(
      sendMessage({
